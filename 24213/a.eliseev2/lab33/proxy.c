...
int proxy_ensure_capacity(proxy_t *proxy) {
    if (proxy->conn_cnt < proxy->conn_cap) {
        return 0;
    }
    if (proxy->conn_cap == 0) {
        proxy->conn_cap = 4;
    ...
    size_t conns_size = sizeof(conn_t) * proxy->conn_cap;
    size_t pollfds_size =
        sizeof(pollfds_t) + sizeof(struct pollfd_pair) * proxy->conn_cap;
    conn_t *conns_new = realloc(proxy->conns, conns_size);
    if (!conns_new) {
        perror("Connection memory allocation failed");
        return 1;
    }
    proxy->conns = conns_new;
    pollfds_t *pollfds_new = realloc(proxy->pollfds, pollfds_size);
    if (!pollfds_new) {
        perror("Pollfds memory allocation failed");
        return 1;
    }
    proxy->pollfds = pollfds_new;
    return 0;
}

int proxy_add(proxy_t *proxy, int fd_in, int fd_out, conn_state_t state) {
    if (proxy_ensure_capacity(proxy)) {
        return 1;
    }
...
int handle_new_client(proxy_t *proxy, struct in_addr remote_addr,
                      in_port_t remote_port) {
    ...
    if (proxy_add(proxy, fd_in, fd_out, state)) {
        if (close(fd_in)) {
            perror("Close failed");
        }
        if (close(fd_out)) {
...
int service_half_duplex(struct pollfd *from, struct pollfd *to, buf_t *buf) {
    ...
    if (to->revents & POLLOUT) {
        ssize_t sent_cnt = send(to->fd, buf->data + buf->send_cnt,
                                buf->recv_cnt - buf->send_cnt, MSG_NOSIGNAL);
        if (sent_cnt == -1) {
            perror("Send failed");
            return 1;
        }
        buf->send_cnt += sent_cnt;
    ...
    if (buf->eof && buf->recv_cnt == 0) {
        // Nothing left to receive, sent everything.
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (proxy.pollfds->listen.revents & POLLIN) {
            if (handle_new_client(&proxy, remote_addr, args.remote_port)) {
                break;
            }
        }
    }
