...
void MessageBuffer::Reader::waitForData() {
    while (!isEnd() && length() == 0) {
        pthread_cond_wait(&buffer->dataCond, &buffer->lock);
    }
}
...
void MessageBuffer::Writer::commit() {
    buffer->readableSize = buffer->data.size() - allocSize;
    pthread_cond_signal(&buffer->dataCond);
}
