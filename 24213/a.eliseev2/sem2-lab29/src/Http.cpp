...
bool Uri::parseClient(const char **data, const char *end, Uri &uri) {
    const char *start = *data;
    if (start == end) {
        return false;
    }
    ...
    uri.host = std::string();
    uri.port = 0;
    uri.raw = std::string(start, end);

    if (!parseAbsolute(data, end, uri)) {
        return false;
    }
    if (*data == end) {
        uri.raw.append("/");
